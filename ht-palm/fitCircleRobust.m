function [z, rad,initguess] = fitCircleRobust(x, varargin)
...
[z rad]= simpleCircleGuess(x);
initguess = [z',rad];

% 2) Nonlinear refinement to miminise geometric error, and compute residual
[z, rad] = fitcircle_geometric(x, z, rad);

% END MAIN FUNCTION BODY

% NESTED FUNCTIONS
    function [z, rad] = fitcircle_geometric(x, z0, r0)
      ...
      rad=u(3);
        ...
        function [f,J] = sys(u)
            ...
            % Objective function
            resid = (sqrt(sum((repmat(u(1:2), 1, m) - x).^2)) - u(3))';
