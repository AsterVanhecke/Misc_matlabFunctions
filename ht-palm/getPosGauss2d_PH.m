function [pos amplitude sigma_x sigma_y]= getPosGauss2d_PH(im, posGuess,  windowSize)
%PH version deletes the halo (below median values)
...
[phot_count, a, normChi2, pos, eccentricity] = freeGaussFitEllipsePH( im, posGuess, windowSize,posThresh, sigmaLim, initguess);
amplitude = a(1);
sigma_x = a(2);
sigma_y = a(3);

%-------------------------------------------------------------------------------------------
function [phot_count a normChi2 pos eccentricity ] = freeGaussFitEllipsePH( im, point_pos, windowSize,posLim, sigmaLim, initguess,varargin)
...
% get rid of the halo by subtracting the median
bgLvl = round(median(fitIm(:)));
fitIm(fitIm<=bgLvl) = bgLvl;
