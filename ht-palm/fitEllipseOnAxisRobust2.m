function [z, a, b,  initguess] = fitEllipseOnAxisRobust2(x, varargin)
...
%% Obtain a linear estimate via circle fit
%%      [z, a, b, alpha] = fitbookstein(x);
%Par = CircleFitByTaubin(x');
%z0     = Par(1:2)';
%a0     = Par(3);
%b0     = Par(3);
%simple circle guess
[z0 a0 b0]= simpleCircleGuess(x);
% Initial conditions
...
if params.fNonlinear
    ...
    % Apply the fit
    [z, a, b] = ...
        fitnonlinear(x, z0, a0, params);
end

% Add the centroid back on
z = z + centroid;

initguess = [(z0+centroid)' a0 b0];
end % fitellipse
...
function [z, a, b, fConverged] = fitnonlinear(x, z0, a0,  params)
% Gauss-Newton least squares ellipse fit minimising geometric distance 
% SH 141022 modified to only fit on-axis ellipses

%WEIGHT='bisquare';
WEIGHT='huber';
% Get initial rotation matrix
m = size(x, 2);
ratioLim = params.ratioLim;

% Get initial phase estimates
maxd = params.maxdiameter;
phi0 = angle( [1 i] *(x - repmat(z0, 1, m)) )';
u0 = [phi0; a0; 1; z0];
lb=[-inf*ones(size(phi0));0;1-ratioLim;[-Inf;-Inf]];
ub=[+inf*ones(size(phi0));maxd/2;1+ratioLim;[Inf;Inf]];
...
z     = u(end-1:end);
    ...
    function [f,J] = sys(u)
        ...
        for i = 1:m
            ...
            % and data point
            resid((2*i-1):(2*i)) = x(:, i) - z - a*[ c(i); bRatio * s(i)];
            
            % Jacobian
            J(rows, i) = -1 * a*[- s(i); bRatio * c(i)];
            %set the off axis rotation to zero- ie constrain to an on-axis ellipse
            J(rows, (end-3:end)) = ...
                [-1*[c(i); bRatio*s(i)], -a*[0; s(i)], [-1 0; 0 -1]];
        end

        if strcmp(WEIGHT,'bisquare');
          s = mad(resid,1);% median absolute deviation of the residuals
          TUNE=4.685;
          r = resid./(TUNE.*s);
          w = (abs(r)<1).*(1 - r.^2).^2;%BISQUARE WEIGHTING
          f = w.*resid; 
        elseif strcmp(WEIGHT,'huber');
          s = mad(resid,1);% median absolute deviation of the residuals
          TUNE=1.345;
          r = resid./(TUNE.*s);
          w = 1 ./ max(1, abs(r));
...
properties = {'constraint', 'maxits', 'tol','maxfuneval','maxdiameter'};
...
%-------------------------------------------------
function [z0 a0 b0]= simpleCircleGuess(x);
