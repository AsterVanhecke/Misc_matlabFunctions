%-------------------------------------------------------------------------%
%                main function: fitAreaFeingoldModBound                         %
%-------------------------------------------------------------------------%
...
function [tc tg wMax fitOut]  = fitAreaFeingoldModBound(t, w2)
...
lb =[0,0,0];
ub =[max(t),Inf,1];
a = lsqcurvefit(@feingoldAreaModel, a0, t, w2, lb, ub);
tc = a(1);
tg = a(2);
wMax = a(3);

tFit = unique(sort(t));
wFit = feingoldAreaModel(a, tFit);
fitOut = [tFit(:), wFit(:)];
end

%----------------------
function wOutSquared = feingoldAreaModel(a, t)
...
isDiv = (t >= tc & t < tg);
tDiv = t(isDiv);
wOutSquared(isDiv) = wMax^2.*( 1 - (tDiv-tc).^2/(tg-tc).^2 );
